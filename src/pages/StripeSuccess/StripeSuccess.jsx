...
const StripeSuccess = () => {
  ...
  useEffect(() => {
    if (user && !hasFetchedUserDetails) {
      dispatch(subStatusCtrl(customerId))
        ...
        .then((updatedUser) => {
          ...
          localStorage.setItem(
            "userInfo",
            JSON.stringify(updatedUser?.payload?.user)
          )
        })
        .catch((error) => {
          console.log(
            "Error in dispatching actions in stripe success page",
            error
          )
        })
    }
  ...
  return (
    <div className="font-poppins w-full items-center justify-center pt-[20%] pb-[50%] bg-gray-900 h-[100%]">
      <div className="flex justify-center">
        <h1 className="block text-white text-2xl pb-10">
      ...
      <div className="flex justify-center">
        <Link
          to={"/"}
          className="px-8 py-3 font-semibold rounded-full bg-sky-400 text-gray-900"
        >
          Go to Homepage
        </Link>
      </div>
